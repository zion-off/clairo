...
const modalBindings: Keybinding[] = [
  { key: 'Esc', label: 'Cancel' },
];

export default function KeybindingsBar({ contextBindings = [], modalOpen = false }: Props) {
  const allBindings = modalOpen
    ? [...contextBindings, ...modalBindings]
    : [...contextBindings, ...globalBindings];

  return (
    <Box flexShrink={0} paddingX={1} gap={2}>
      {allBindings.map((binding) => (
        <Box key={binding.key} gap={1}>
          <Text bold color={binding.color ?? 'yellow'}>{binding.key}</Text>
